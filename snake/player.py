...
from snake.settings import BLOCK_SIZE, UP, DOWN, LEFT, RIGHT, GAME_WIDTH, GAME_HEIGHT
...
class Player:
    ...
    corner = {
        (UP, RIGHT): 180, (UP, LEFT): 90,
        (DOWN, RIGHT): 270, (DOWN, LEFT): 0,
        (RIGHT, UP): 0, (RIGHT, DOWN): 90,
        (LEFT, UP): 270, (LEFT, DOWN): 180
    }

    def __init__(self, sprites: Dict[str, Any], start_pos: Tuple[int, int] = (100, 100), 
                 start_direction: Tuple[int, int] = RIGHT, player_id: int = 1):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        for i in range(1, len(self.body) - 1):
            ...
            if next_seg.direction == current.direction:
                body_sprite = rotate(self.sprites["body"], self.angle[current.direction])
            else:
                body_sprite = rotate(
